...
The project uses four GitHub Actions workflows with a three-tier artifact promotion model:

| Workflow | Trigger | Environment | Artifacts |
|----------|---------|-------------|----------|
| **CI** (`ci.yml`) | Push/PR to main | `dev` | Docker `:dev` + `:dev-<sha>` (main only) |
| **Release** (`release.yml`) | `vX.Y.Z-rc.N` tag | `staging` | Docker `:X.Y.Z-rc.N`, pre-release GitHub Release |
| **Release** (`release.yml`) | `vX.Y.Z` tag | `production` | Docker `:X.Y.Z`, `:X.Y`, `:X`, `:latest`, GitHub Release |
| **Promote** (`promote.yml`) | Manual dispatch | `production` | Zero-rebuild tag promotion from RC |
| **Security** (`security.yml`) | Push/PR + weekly | — | CodeQL, Trivy, SBOM, Scorecard |

See [ENVIRONMENTS.md](ENVIRONMENTS.md) for environment protection rules and setup.
...
The CI pipeline is optimized for **< 10 min full pass** (cached) and **< 3 min incremental**:

| Layer | Tool | Key strategy | Scope |
|-------|------|-------------|-------|
| **Compiler output** | ccache | Hash of `CMakeLists.txt`, `sdk/**`, `tests/**`, `examples/**` with progressive restore-keys | CI, Release, Security (CodeQL) |
| **Python packages** | pip cache | Built-in `actions/setup-python` cache | Wheel smoke, Release wheels |
| **Apt packages** | `actions/cache` | Per-runner `~/apt-cache` | Build jobs |
| **Docker layers** | BuildKit GHA cache | Scoped per image variant (`runtime`/`dev`) | Docker smoke, Release, Security |

All four CI jobs (`build-and-test`, `deb-smoke`, `wheel-smoke`, `docker-smoke`) run **independently in parallel** — no serial bottleneck.
...
**Path A — Promoted release (recommended for production governance):**
```bash
# 1. Ensure CMakeLists.txt project(VERSION X.Y.Z) matches desired release
# 2. Tag an RC and push:
git tag v1.2.3-rc.1
git push origin v1.2.3-rc.1

# 3. Validate the RC release in staging
# 4. Promote to production (Actions → Promote → Run workflow):
#    Source tag: v1.2.3-rc.1
#    This re-tags Docker images (no rebuild) and creates production release
```

**Path B — Direct release (full rebuild):**
```bash
# 1. Ensure CMakeLists.txt project(VERSION X.Y.Z) matches desired release
# 2. Tag and push:
git tag v1.2.3
...
# Full build + test + Docker push + Release (requires production approval)
```

#### Artifact promotion model

```
Dev (:dev-<sha7>)           ← CI push to main (auto)
  │
Staging (:X.Y.Z-rc.N)      ← RC tag push (auto)
  │
  ├─► promote.yml           ← Manual (zero-rebuild, crane copy)
  │     OR
  └─► release.yml           ← vX.Y.Z tag (full rebuild)
  │
Production (:X.Y.Z :X.Y :X :latest)  ← Requires reviewer approval
```

**Immutable artifact policy:**
- Production Docker tags are write-once (verified before promotion)
- GitHub Releases cannot overwrite existing ones
- Promoted artifacts are byte-identical to RC (verified via SHA256 + cosign)
- Digest is preserved through `crane copy` (manifest-level operation)

### Security Pipeline (`security.yml`)

Runs on every push/PR to `main`/`develop` + weekly Monday 06:00 UTC schedule:

| Job | Tool | Purpose |
|-----|------|---------|
| **CodeQL** | `github/codeql-action` | C++ & Python static analysis (security + quality queries) |
| **Dependency Review** | `actions/dependency-review-action` | PR-only: blocks high/critical CVEs and copyleft licenses |
| **SBOM (source)** | Syft (`anchore/sbom-action`) | SPDX + CycloneDX SBOMs from source tree |
| **Container Scan** | Trivy (`aquasecurity/trivy-action`) | CVE scanning of runtime + dev Docker images → SARIF to GitHub Security |
| **Secret Scan** | TruffleHog | Detects leaked credentials in commit history |
| **Scorecard** | OpenSSF Scorecard | Supply chain health assessment (branch protection, CI, etc.) |

All results upload to the GitHub Security tab (Code Scanning alerts).
...
| SBOM | `thunderbird-sdk.spdx.json` (SPDX) | — |

### Security & Signing

The project implements defense-in-depth across the entire supply chain.
See [SECURITY.md](SECURITY.md) for full details including vulnerability reporting.

#### Supply chain security

| Layer | Tool | Coverage |
|-------|------|----------|
| **Static analysis** | CodeQL | C++ & Python on every push/PR + weekly |
| **Dependency scanning** | Dependabot + Dependency Review | Actions, pip, Docker base images |
| **Container scanning** | Trivy | CRITICAL/HIGH CVEs block release |
| **Secret detection** | TruffleHog | Full commit history scan |
| **Supply chain health** | OpenSSF Scorecard | Automated OSSF assessment |

#### Artifact signing & provenance

| Artifact | Signing method | Provenance |
|----------|---------------|------------|
| Docker images | Cosign keyless (Sigstore OIDC) | BuildKit SLSA provenance + SBOM attestation |
| `.deb` packages | SHA256SUMS + cosign blob | `actions/attest-build-provenance` (SLSA L2+) |
| `.whl` packages | SHA256SUMS + cosign blob | `actions/attest-build-provenance` (SLSA L2+) |
| Checksums file | Cosign blob signature + certificate | — |
| Source + containers | Syft SPDX SBOM | Cosign attestation |

#### SBOM strategy

- **Source-level SBOM** — Generated on every push via Syft (SPDX + CycloneDX)
- **Container SBOMs** — Generated per image variant and attested via `cosign attest`
- **Release SBOM** — Attached to every GitHub Release as `thunderbird-sdk.spdx.json`
...
- **Per-job escalation**: Only jobs that need `packages: write`, `id-token: write`,
  or `attestations: write` declare those permissions
- **No long-lived secrets**: All signing uses Sigstore OIDC keyless (no PATs or deploy keys)
